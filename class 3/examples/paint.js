			...
			function init() {
               ...
               // camera.position.set(0, 1, 3);
				...
				var material = new THREE.MeshPhongMaterial({color: `hsl(${Math.random() * 360}, 40%, 60%)`});
      			  material.flatShading = true;
				
				var floor = new THREE.Mesh( geometry, material );
				floor.rotation.x = - Math.PI / 2;
				floor.position.set(0,-1,-4);
				scene.add( floor );
			...
			function handleController( controller ) {
				...
				if ( pivot ) {
					...
					matrix1.lookAt( point2, point1, cameraTarget );
